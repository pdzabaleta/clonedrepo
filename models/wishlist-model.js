...
async function getWishlistItem(account_id, inventory_id) {
  try {
    const sql = "SELECT * FROM wishlist WHERE account_id = $1 AND inventory_id = $2";
...
async function removeFromWishlist(account_id, inventory_id) {
  try {
    const sql = "DELETE FROM wishlist WHERE account_id = $1 AND inventory_id = $2 RETURNING *";
    const result = await pool.query(sql, [account_id, inventory_id]);
    return result.rows[0];
  } catch (error) {
...
module.exports = { addToWishlist, getWishlistItem, getWishlist, removeFromWishlist };
